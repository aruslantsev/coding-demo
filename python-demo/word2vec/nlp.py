...
class Word2Vec:
    ...
    def fit(self, X, embedding_size=128, window_size=5, epochs=10, lr=0.1, vocab_size=1000, train_size=500):
        ...
        if vocab_size is not None and vocab_size < len(self.corpus):
            self.corpus = sorted(counts.keys(), key=lambda tok: counts[tok], reverse=True)[:vocab_size]
        else:
    ...
    def load(self, path):
        ...
        token2idx = model_state['token2idx']
