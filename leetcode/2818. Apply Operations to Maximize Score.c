...
int maximumScore(int* nums, int numsSize, int k) {
    ...
    int *stack, stack_ptr = -1;
    stack = (int *) malloc(numsSize * sizeof(int));
    for (int i = 0; i < numsSize; i++) {
        while ((stack_ptr >= 0) && (prime_scores[stack[stack_ptr]] < prime_scores[i]))
            next_dominant[stack[stack_ptr--]] = i;
        if (stack_ptr >= 0)
            prev_dominant[i] = stack[stack_ptr];
        stack[++stack_ptr] = i;
    }
    free(stack);

    for (int i = 0; i < numsSize; i++) {
        num_subarrays[i] = (long long) (next_dominant[i] - i) * (i - prev_dominant[i]);
    }
