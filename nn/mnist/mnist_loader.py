...
class MNISTDataset(Dataset):
    def __init__(self, lst, mnist, image_transforms):
        self.lst = lst
        self.mnist = mnist
...
def train_test_split(train_ratio=0.8, mnist)
    all_images = list(range(len(mnist['targets'])))
    train_idx = sorted(np.random.choice(all_images, int(train_ratio * len(mnist['targets'])), replace=False).tolist())
    test_idx = [image for image in all_images if image not in train]
